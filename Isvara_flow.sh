...
if [ $a -eq 1 ]; then echo "1"; else echo "0"; fi       #如果$a等于1，则输出1，否则输出0
    ...
    #[]中的操作符可以使用-a表示与，-o表示或，!表示非；也可以使用与(( ))类似的操作符，如&&表示与，||表示或，!表示非
    #如果习惯将多种条件组合在一起且写在同一个[]中，需要注意条件判断的先后。一般而言，是从左往右进行判断，如果需要改变判断顺序，可以使用()括起来，()中的条件优先判断
...
#形如上面两行代码，也可以写成如下形式，个人更推荐如下形式
if [ $a -ne 1 ] && [ $b -ne 1 ]; then echo "1"; else echo "0"; fi                               #如果$a不等于1并且$b不等于1，则输出1，否则输出0
if [ $a -ne 1 ] || [ $b -ne 1 ]; then echo "1"; else echo "0"; fi                               #如果$a不等于1或者$b不等于1，则输出1，否则输出0
