...
until [ $a == 0 ]                                   #until表示直到a等于0才停止
    ...
    #c=`expr $b - 2`                                 #c表示当前正在运行的任务数
    if [ $b -lt 20 ]                                #如果当前正在运行的任务数小于20，则继续提交任务
        then
        d=`expr 20 - $b`
        ...
        for e in `cat qsub.list`
            do
            cd $e
            qsub -cwd -q st.q -P P17Z10200N0246 -l vf=30g,num_proc=4 -binding linear:4 *.sh
            cd /ldfssz1/ST_HEALTH/P17Z10200N0246/lizhuoran1/ERP004605
            echo "$e has been qsub" >> qsub.log
        done
    else
        f=`date`
        echo $f >> qsub.log
        echo "There are $c jobs running, please wait" >> qsub.log
    fi
    sleep 2m                                        #sleep 2m表示休眠2分钟
...
echo "All jobs have been qsub" >> qsub.log
