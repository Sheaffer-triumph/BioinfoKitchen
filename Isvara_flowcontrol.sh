#条件判断
if [ $a -eq 1 ]; then echo "1"; else echo "0"; fi       #如果$a等于1，则输出1，否则输出0；[ ]中的空格是必须的，否则会报错；[]与(( ))的区别是[]是bash的内建命令，而(( ))是bash的语法结构，[]中只能使用特定的操作符进行比较，而(())可以使用算术操作符，不能使用[]中的操作符；[]中的变量需要加$符号，而(())中的变量不必须要加$符号，但是建议加上，以一致。
