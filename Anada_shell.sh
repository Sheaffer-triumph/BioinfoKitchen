...
cd-hit-est -i A.fa -o B.fa -c 0.95 -aL 0.9 -M 16000 -T 8    
    ...
    #-M 16000表示最大内存为16G
    #-T 8表示使用8个线程
    #在使用cd-hit工具时，有时碰见处理序列的长度超过了最大序列长度。此时会有warninf提示，可能伴随着无输出。解决方案是重新使用git clone下载cd-hit工具，然后在cdhit文件夹下重新编译安装，安装时需要指定最大序列长度，如make MAX_SEQ=1000000，此时最大序列长度为1000000
