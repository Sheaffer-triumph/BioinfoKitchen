...
echo file1.txt file2.txt file3.txt | xargs -I {} mv {} {}.bak   #将echo的输出作为参数传递给xargs命令，xargs默认将参数传递给后面的命令，-I {}表示将参数传递给{}，{}可以换成其他字符，但在使用时需要保持一致，注意不要与别的命令冲突；这种情况下使用命令，无法使用bashrc里的alias过的命令
...
cd-hit-est -i A.fa -o B.fa -c 0.95 -aL 0.9 -M 16000 -T 8    
    ...
    #-M 16000表示最大内存为16G
    #-T 8表示使用8个线程
    #在使用cd-hit工具时，如果处理序列的长度超过了最大序列长度。此时会有warning提示，可能伴随着无输出。可使用git clone重新下载cd-hit，并在cdhit目录下重新编译安装，安装时需要指定最大序列长度，如make MAX_SEQ=1000000，此时最大序列长度为1000000
