...
while read i; do qsub -cwd -q st.q -P P17Z10200N0246 -l vf=80g,num_proc=8 -binding linear:8 megahit.sh $i ; done < list.txt
qstat                       #显示所有已投递的任务
...
cd-hit-est -i A.fa -o B.fa -c 0.95 -aL 0.9 -M 16000 -T 8    
    ...
    #-M 16000表示最大内存为16G
    #-T 8表示使用8个线程
    #在使用cd-hit工具时，如果处理序列的长度超过了最大序列长度。此时会有warning提示，可能伴随着无输出。可使用git clone重新下载cd-hit，并在cdhit文件夹下重新编译安装，安装时需要指定最大序列长度，如make MAX_SEQ=1000000，此时最大序列长度为1000000
